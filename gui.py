...
# Load media button frame
load_button_frame = ctk.CTkFrame(window, fg_color=theme_color)
...
# Algo submenu frame
floyd_steinberg_submenu = ctk.CTkFrame(window, fg_color=theme_color)
...
algo_label_frame = ctk.CTkFrame(window, fg_color=theme_color)
algo_label_frame.place(
    relx=0.5, rely=0.515, relwidth=0.3, relheight=0.03, anchor="center"
)

# Algorithm dropdown label
algo_label = ctk.CTkLabel(algo_label_frame, text="Dithering Algorithm", anchor="w")
algo_label.pack()

# Algorithm submenu label frame
algo_submenulabel_frame = ctk.CTkFrame(window, fg_color=theme_color)
...
def show_submenu(selected_value):
    ...
    elif selected_value == "Floyd-Steinberg":
        submenu_dropdown_frame.place_forget()
        floyd_steinberg_submenu.place(
            relx=0.5, rely=0.65, relheight=0.09, anchor="center"
        )
...
downscale_slider.pack(fill=ctk.X, expand=True)
